...
class ndjsonDemo(AbstractDemo):
    ...
    def load_file(self, file_name, file_extension):
        print("Loading ndjson file: " + file_name + "." + file_extension)
        with open(self.input_directory + file_name + "." + file_extension, "r") as file:
            return file.readlines()

    def create_demo(self):
        print("Creating demo for ndjson file")
        count = 0
        while count < self.number_of_object:
            self.output_data.append(self.file[count])
            count += 1
...
class csvDemo(AbstractDemo):
    ...
    def load_file(self, file_name, file_extension):
        print("Loading csv file: " + file_name + "." + file_extension)
        with open(self.input_directory + file_name + "." + file_extension, "r") as file:
            return file.readlines()

    def create_demo(self):
        print("Creating demo for csv file")
        count = 0
        while count < self.number_of_object + 1:  # the first line is the header
            self.output_data.append(self.file[count])
            count += 1
